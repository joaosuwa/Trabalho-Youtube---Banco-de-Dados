...
# Tabela de vídeos não curtidos por uma conta especidifcada
def listar_videos_nao_curtidos_por_conta(cursor, id_conta):

    """
    Retorna todos os vídeos (id e título) que a conta especificada
    ainda não curtiu/descurtiu.
    """
    comando = """
    SELECT
        v.id_publicacao,
    ...
    JOIN Publicacao p ON v.id_publicacao = p.id_publicacao
    JOIN Canal c ON p.id_canal = c.id_canal
    WHERE v.id_publicacao NOT IN (
        -- Subconsulta: Lista todos os IDs de publicação que a conta JÁ curtiu
        SELECT cur.id_publicacao
        FROM Curtida cur
        WHERE cur.id_conta = %s
    )
    ORDER BY v.titulo
    LIMIT 100; -- Limitar o resultado para não sobrecarregar
    """
    # O parâmetro id_conta será inserido na cláusula WHERE (usado na subconsulta)
    cursor.execute(comando, (id_conta,))
    return cursor.fetchall()

# Dado um id de publicação retornamos as curtidas associadas
def listar_curtidas_por_publicacao(cursor, id_publicacao):

    """
    Retorna a lista de todas as curtidas (likes/dislikes) dadas a uma
    publicação específica (vídeo, livestream, etc.).
    """
    comando = """
    SELECT
        co.nome AS nome_curtidor,
        co.id_conta AS id_curtidor,
        CASE WHEN c.dislike = TRUE THEN 'DISLIKE' ELSE 'LIKE' END AS reacao
    FROM Curtida c
    JOIN Conta co ON c.id_conta = co.id_conta
    WHERE c.id_publicacao = %s;
    """
    cursor.execute(comando, (id_publicacao,))
    return cursor.fetchall()

# Tabela com todas as contas
def listar_todas_contas(cursor):
    comando = """
    SELECT id_conta, nome
    FROM Conta
    ORDER BY nome;
    """
    cursor.execute(comando)
...
# tabela que devolve quantos likes e dislikes houveram
def conta_likes_dislikes_publicacao(cursor, id_publicacao):
    comando = """
    SELECT 
        SUM(CASE WHEN dislike = FALSE THEN 1 ELSE 0 END) AS total_likes,
        SUM(CASE WHEN dislike = TRUE THEN 1 ELSE 0 END) AS total_dislikes
    FROM Curtida
